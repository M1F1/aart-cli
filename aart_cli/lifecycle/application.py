...
from aart_cli.install_state.model import EffectProof, InstallationRecord, InstallState
...
from aart_cli.installation.application import (
    InstallApplyPorts,
    InstallReadPorts,
    finalize_install,
    prepare_install,
...
def _recorded_subscription_current(
    ...
    return bool(
        ...
        # against -- is intact and still serving.  That is exactly what a refused `aart-cli source
        # sync` leaves behind, and excluding it made one invalid upstream revision report every
        # installation from that source as `source-unavailable` and gave `aart-cli marketplace update`
...
def _retired_effect_update(
    ...
    detail = (
        ...
        + ", ".join(retired)
        + "; migrate transactionally with `aart-cli marketplace uninstall "
        + coordinate
        + " --profile "
        + record.profile
        + scope
        + " --yes`, then `aart-cli marketplace install "
        + coordinate
        + " --profile "
        + record.profile
        + scope
