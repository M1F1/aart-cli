...
from aart_cli.domain.receipts import InstalledRecord, receipt_profiles
...
from aart_cli.io import fs
from aart_cli.io.receipt_store import LocalReceiptStore
from aart_cli.model import InstallScope, SetupState, SetupStateRecord
...
from aart_cli.setup_render import receipt_payload
from aart_cli.setup_runtime import production_runtime, rollback_record
from aart_cli.setup_undo import plan_undo, undo_digest, undo_payload
from aart_cli.setup_verify import plan_verification, verification_payload, verify_claims
from aart_cli.setup_verify_probes import local_probes
...
def _bound(location: ReceiptLocation, record: SetupStateRecord) -> Result[ReceiptLocation]:
    ...
    if location.scope != record.scope:
        return _error(
            ...
            (
                f"read it with: aart-cli marketplace receipt show <coordinate> --scope {record.scope}",
            ),
        )
    ...
    *,
    ...
    if len(matches) > 1:
        ...
        return _error(
            ...
            ("name one with: aart-cli marketplace receipt show <coordinate>",),
        )
    record = matches[0]
...
def _locate_legacy(
    *,
    ...
    missing = _error(
        ...
        (
            "list what is installed with: aart-cli marketplace status",
            "install it with: aart-cli marketplace install",
        ),
    )
    ...
    if len(candidates) > 1:
        ...
        return _error(
            ...
            ("name one with: aart-cli marketplace receipt show <coordinate> --profile <profile>",),
        )
...
def unsupported_action(action: str | None) -> Err:
    return _error(
        ...
        (
            "read a persisted record with: aart-cli marketplace receipt show <coordinate>",
            "check whether it is still true with: aart-cli marketplace receipt verify <coordinate>",
            "reverse what it recorded with: aart-cli marketplace receipt undo <coordinate>",
        ),
    )
    ...
    return Err(
        (
            Diagnostic(
                ...
                or ("list what is installed with: aart-cli marketplace status",),
            ),
        )
    )
