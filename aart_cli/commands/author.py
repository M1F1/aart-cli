"""The authoring group: `aart-cli author init` writes a skeleton an author edits down.
...
from aart_cli.authoring.check import (
    AuthorCheckReport,
    ManifestVerdict,
...
from aart_cli.domain.diagnostics import Diagnostic, diagnostic_to_data
from aart_cli.domain.result import Err
from aart_cli.io.author_workspace import (
    WORKING_TREE_ALIAS,
...
_READ_THE_ACTIONS = "Run `aart-cli author --help` for the actions this build has."
...
def _run_init(request: Request) -> int:
    ...
    for relative in written.value.paths:
        print(f"  {relative}")
    print("Delete what you do not need, uncomment what you do, then `aart-cli author check`.")
    return OK
