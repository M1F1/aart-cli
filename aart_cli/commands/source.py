...
from aart_cli.application.configuration import save_user_configuration_checked
from aart_cli.application.source_management import (
...
from aart_cli.sources.runtime import (
...
def _list(request: Request) -> int:
    ...
    if request.json:
        ...
        print("No configured sources. Add one with `aart-cli source add --help`.")
    else:
        for item in items:
...
def _selected_sources(
    ...
    if not selected:
        return _failure(
            ...
            "run `aart-cli source list --json` to see configured aliases",
        )
    return Ok(selected)
...
def sync_configured_sources(request: Request) -> Result[tuple[SynchronizedSource, ...]]:
    ...
    This is the transaction behind both `aart-cli source sync` and screen 21's reviewed refresh
...
def _sync(request: Request) -> int:
    ...
    for source in selected.value:
        ...
        if isinstance(synchronized, Err):
            ...
            # the `source resubscribe` line is produced.
