...
from aart_cli.curation.runtime import (
...
from aart_cli.io.registry_publication import publish_registry_commit
from aart_cli.io.registry_workspace import (
...
from aart_cli.protocol.registry_models import RegistryManifest
...
from aart_cli.registry_commands.model import RegistryQualityReport
...
from aart_cli.registry_maintenance.discovery import discover_vendor_candidates
...
_READ_THE_ACTIONS = ("`aart-cli registry --help` lists the actions this command accepts",)
_NAME_THE_ARTIFACT = (
...
def _run_discover(request: Request) -> int:
    ...
    if request.json:
        ...
        print(
            "review each `accept` field, then run `aart-cli registry vendor-batch --manifest FILE`"
        )
    return _common.OK
    ...
    if (
        ...
        return _emit_error(
            ...
            _error(
                ...
                (
                    "commit or discard author checkout changes; `aart-cli registry scan --help` "
    ...
    if not set(request.promotion_candidate_ids) <= set(by_id):
        return _emit_error(
            ...
            _error(
                ...
                (
                    "use `aart-cli registry scan --help`, run a fresh scan with explicit boundaries, "
...
_NAME_THE_SCAN = (
    "name the repository to look at: `aart-cli registry adopt --source DIR --url URL --ref REF`",
)
_NAME_THE_ADOPTED = (
    "name one adopted artifact, as `KIND/NAME@VERSION`; "
    "`aart-cli registry adopt --source DIR --url URL --ref REF` without `--artifact` lists what a "
    "repository declares",
)
