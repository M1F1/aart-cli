...
from aart_cli.compiler.graph import ArtifactLifecycle, MarketplaceArtifact, MarketplaceGraph
...
from aart_cli.configuration.schema import organization_policy_bytes
...
from aart_cli.protocol.json import JsonArray, JsonObject, JsonValue, canonical_json_bytes
from aart_cli.protocol.semver import SemVer, version_bounds_label
    ...
    *,
    ...
    if not scope:
        ...
        return _error(
            ...
            message,
            remediation=(
                f"aart-cli source add --alias {alias} --kind registry-git --location <url>",
                "aart-cli marketplace uninstall, if you are removing what this source installed",
            ),
        )
    if all(source.snapshot_digest is None for source in scope):
        ...
        if offline:
            return _error(
                ...
                remediation=(
                    ...
                    f"aart-cli source sync --alias {scope[0].alias}, while connected",
                ),
                interactive=(
        ...
        return _error(
            ...
            remediation=(f"aart-cli source sync --alias {scope[0].alias}",),
            interactive=(
    ...
    listing = "aart-cli marketplace list" + (
