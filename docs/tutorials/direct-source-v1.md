...
7. Use `aart-cli marketplace status`, its fetch-free `--offline` form, reviewed
   `aart-cli marketplace update`, and `aart-cli marketplace uninstall` for the recorded subscription.
   `aart-cli doctor` reports the whole machine at once rather than one subscription.
