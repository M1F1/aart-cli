...
| CP-16 Doctor/supportability | IN PROGRESS | Step 1 adds the top-level, read-only `aart doctor` over real canonical project and user installations (D-139). One durable observation feeds both the accepted screen-29 health projection and `prepare_configured_repair`, so the JSON carries measured component drift and the same minimal plans lifecycle repair would use rather than a command-local planner. Six E2E tests install two approved-registry Skills through the public command and hold healthy/no-plan, one-drift/one-plan, human detail, both scopes, source-independent inspection and a structured unreadable-receipt refusal. The parser absence was the RED; healthy-plan and project-only mutations independently turn their claims red. Fresh scoped mutation run: 104/132 killed, 28 reviewed survivors, none untested. `make quality` is green with 3,243 tests, 1 skipped and 85.34% branch coverage; `make integration` is separately green with 279 E2E tests. | Step 1 is VERIFIED. The first command reports plans but applies none. Offline readiness (B-051), reviewed repair entry points and Activity/Receipt/configuration/credential diagnostics remain in CP-16. |
