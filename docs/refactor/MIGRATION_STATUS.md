...
| CP-17 Git-backed live acceptance | IN PROGRESS | Step 1 is VERIFIED and joins the first previously isolated stages (D-146). `git_source_publication_e2e_test` builds an ordinary repository, obtains a complete `SourceCandidate` from the system Git adapter, passes that exact value through validation and atomic publication, and reads the same candidate through a fresh store reader. The durable revision is the real `git rev-parse` SHA rather than `"a" * 40`; an unmoved second acquisition creates no snapshot and a new commit becomes current. The placeholder mutation turns both scenarios red, and a fresh scoped mutation run tightened full-value and exact-boolean assertions. No production code changed and local transport remains enabled only at the direct adapter test boundary. All nine quality gates are green with 3,293 tests, one skipped and 85.38% branch coverage; `make integration` is separately green with 323 E2E tests. | Step 2 is next: a valid remote hostname in real user configuration, with only the acquisition port substituting local Git transport, then public sync, Marketplace listing and an install receipt carrying the real commit. The configuration and transport refusals remain unchanged. |
