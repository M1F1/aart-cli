...
| CP-18 Migration/release gate | IN PROGRESS | Step 1 evidences INV-071 against the import graph rather than against a declaration. Two checks already existed and neither held the claim: `dev_tools_test` reads `[project] dependencies` and asserts `[]`, and `scripts/packaging_check.py` builds the wheel and refuses a non-extra `Requires-Dist`. Both are statements about a declaration, and INV-071 is about a dependency graph. A runtime module that imports a development tool *inside a function body* declares nothing, adds no `Requires-Dist`, ships inside the wheel and violates the invariant on the first call. That was measured rather than supposed: inserting `import hypothesis` into a function body in `agent_artifacts/application/installed_state.py` left `dev_tools_test` and `packaging_test` green (36 passed, 1 skipped) and left `packaging_check` reporting `packaging check OK`. `tests/runtime_purity_test.py` parses every module under `agent_artifacts/` with `ast` and asserts none imports a development tool, the test suite or the gate scripts. Two choices carry it: the forbidden set is read out of Poetry's dev group at test time rather than hardcoded, so a tool added tomorrow is covered without anyone remembering; and `ast` rather than import-time introspection, because walking `sys.modules` sees only module-level imports and the function-body case is both the realistic shape of such a leak and the only one that defeats both incumbent checks. Two of its four tests are guards on the test itself -- the forbidden set must be non-empty and the tree must hold more than 100 sources -- because an absence assertion over an empty tree passes for the wrong reason (D-149). The mutation turned the claim's own test red and left the four others green. The first draft read the dev group with `tomllib` and mypy refused it, which is not a stub gap: `requires-python` is `>=3.10` and `tomllib` arrived in 3.11, a constraint `dev_tools_test` already records for `poetry.lock`. A test for this invariant that runs on only some supported interpreters is the wrong shape, so the group is read by a flat-table shortcut held by a claim of its own, and a second mutation running that shortcut past its table header turns exactly that test red. Deriving the set also found B-068: `mutmut` is declared in the dev group but absent from `poetry.lock`, so the provisioning path omits it and `make mutants` cannot run in an environment built that way -- advisory tooling by D-134, so backlog rather than a blocker. INV-071 moves to EVIDENCED. The slice survey also found that most of the remaining CP-18 rows are stale bookkeeping rather than missing work: `enterprise_ci_template_test.py` and `release_workflow_test.py` already hold much of INV-072-080 through the emitted workflows, which step 2 audits row by row. Step 2's first row justified the audit twice over. INV-077 was two situations, not one. This repository's `pr-check` aggregate was already correct -- one stable name, `if: always()`, an explicit failure when both arms skip, an allowlist rather than a check for `failure` -- and held by nothing: `quality_gates_test` covers the matrix default and the composite-action delegation, while the shell that decides the verdict, which is the one thing branch protection depends on, was covered by no test at all. `tests/aggregate_gate_test.py` extracts that script from the YAML and executes it under `bash` for every combination of `needs.*.result`, which is INV-076 collecting on its own promise that logic of this kind stays runnable outside GitHub Actions -- a far stronger claim than grepping YAML for the substrings a correct script would contain. The second situation was a shipped defect: the CI `aart registry init` writes had no aggregate at all, only `registry-quality` and `registry-quality-private-image`, two container shapes of which exactly one ever runs, each a matrix over `compatibility: [minimum, latest]`. A registry owner protecting `main` therefore had no name stable across configurations, and naming an arm their deployment skips is worse than useless because GitHub counts a skipped required check as satisfied -- the rule passes precisely when nothing was proven. `_aggregate()` now emits `registry-quality-gate` with the same verdict logic, no container and no Python, since the job branch protection depends on must not fail for a reason unrelated to the gates; the registry README gained a `Protecting main` section naming it (INV-075), held to the emitted YAML by a drift test that reads the name rather than repeating it. Five mutations, each red only on the test stating the claim it breaks: deleting the both-skipped branch, admitting `cancelled` to the allowlist, dropping `if: always()`, deleting the README section while leaving the workflow intact, and giving the emitted gate a container. INV-077 moves to EVIDENCED for both this repository's CI and the CI it ships to others (D-153). Nine quality gates green, 342 E2E tests green. | Continue step 2 with INV-072, 073, 074, 075, 076, 078, 079, 080, audited the same way; two rows in and both found something, so do not assume the rest are bookkeeping. Then legacy removal, docs reconciliation, traceability completion and the closing gates. |
