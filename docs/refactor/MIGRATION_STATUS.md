...
| CP-17 Git-backed live acceptance | IN PROGRESS | Opened with its survey done and no code written yet; the slice document is `docs/refactor/slices/CP-17-git-backed-live-acceptance.md`. The survey's finding is that every stage of the chain already has end-to-end coverage -- a real repository is really cloned by `git_source_adapter_test`, a real MCP server is really launched by `mcp_stdio_e2e_test`, and sync, promotion, marketplace install, collection membership, update, drift, repair, uninstall and purge each have their own E2E file, 46 in total -- and that **nothing joins them**: every consumer-side fixture publishes its snapshot into the source store by hand and records `"a" * 40` as the commit, so each stage is proven against a precondition a test synthesized for it rather than against the artifact the previous stage really emits. That placeholder is the tell, and it is CP-15's argument -- a safeguard at a seam is worth what the verb an operator runs makes of it -- raised to the scale of the whole chain. Two refusals were measured before any code was designed, and neither may be widened to make a test hermetic. The transport allowlist: `application/sources.py:285` leaves `allow_local_transport` at its default `False` and nothing in the package ever passes `True`, so `aart source sync` cannot clone a local repository. Behind it, and stricter, the configuration schema refuses a local Git location outright -- `parse_user_configuration` runs `git_location_parts` on every non-local source, and both `file:///...` and `/path/to/repo.git` return `None` -- so a consumer cannot even be *configured* with one. That fixes the shape of the work rather than blocking it: source identity, the `source_instance_id` the store is keyed by, derives from the configured location, so the configuration must name the real remote host and the substitution belongs at the transport, where the port receives the genuine `GitSnapshotRequest` and clones a real local repository instead of reaching the network. Configuration parsing, identity, publication and every consumer stage downstream then run unmodified against real Git output. Standing in for the network is what a port is for; standing in for the allowlist's verdict would be weakening a security boundary to make a test pass, which the execution contract forbids. | Step 1 is the next RED: a real Git repository cloned by the real adapter, whose returned entries and `git rev-parse` commit become the published snapshot in place of the placeholder. |
