...
# `DOC011` used to live here: the changelog's headings decided the next version, so a heading in
# the wrong shape was a version that came out wrong.  The release engine reads commit messages
# now, not headings, so the shape of a section is prose again and there is nothing for a gate to
# enforce about it.  The rule it enforced went with `scripts/changelog.py` (INV-084).
