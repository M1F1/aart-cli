...
Counts may accompany content and may not replace it. These tests are written
...
def test_the_count_survives_but_comes_after_the_content_it_used_to_replace() -> None:
    ...
    assert len(lines) > 1, "a count must not replace the content it counts"
...
def test_the_recovery_note_reaches_this_surface_too() -> None:
    """The wizard prints these, and so must this path.

    The Docker note names Docker, the tag, and what a rollback would do to the image; everyone who
    runs setup from the command line needs to see it.
    """
...
def test_both_surfaces_print_the_same_block_for_the_same_item() -> None:
    """The strongest form of surface parity: not similar output, the same lines.
...
def test_laf63_a_prefixed_credential_name_is_redacted() -> None:
    # A pattern anchored on `\b` would never match a vendor-prefixed name: a bare `TOKEN=` would
    # redact and every prefixed form would not, and those are the forms real recipes use. The same
    # pattern redacts the persisted record, so a miss is a credential reaching disk, not only a
    # terminal.
...
def test_a_digest_is_not_mistaken_for_a_credential() -> None:
    # The accepted limit: detection is by shape, never by
...
def test_the_reload_next_step_is_one_row_for_the_run_not_one_per_artifact() -> None:
    """One reload reminder per run on the `--json` path, as in the wizard.

    The row carries no artifact key: it never described one. Reloading a shell is a
